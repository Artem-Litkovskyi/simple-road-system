...
{
    public class SRSDynamicRoadTerrainUpdater : MonoBehaviour
    {
        ...
        public void UpdateTerrains()
        {
            ...
            foreach (var terrain in FindObjectsByType<Terrain>(FindObjectsInactive.Include, FindObjectsSortMode.None))
            {
                ...
                {
                    foreach (var curvePointData in curvePointsData)
                    {
                        var localPoint = terrain.transform.InverseTransformPoint(curvePointData.position);
